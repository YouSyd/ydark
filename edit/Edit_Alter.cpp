...
2、支持文本内容居中
3、自带Label.
4、MultiLine垂直滚动条绘制(其内嵌自绘白色边框会导致一些问题，暂未解决)
5、基于comctrl 6.0
*/
#include<windows.h>
#include<windowsx.h>
...
int Edit_NCPaint(HWND hwnd)
{
    ...
    if(ES_MULTILINE!=((UINT)GetWindowLongPtr(hwnd,GWL_STYLE)&ES_MULTILINE)) {
...
int Edit_NCCalSize_MultiLines(HWND hwnd,LPNCCALCSIZE_PARAMS calc)
{
    ...
    //处理后
    //0:新客户区 1:新 2:老
    client_rect_new = {rect_new.left+(IsBorder?es->margin_border:0),//-(Edit_IsShowVScroll(hwnd)?SCROLLBAR_PIXLS:0),
    ...
    case WM_MOUSEMOVE: {
        ...
        if(GetCapture()==hwnd) {
            if(es->scroll_state==THUMB_VCLICK) {
                ...
                int cur_pos=es->dragpos.y+(pt.y-es->dragpt.y)*1.0/((rc[3].top-rc[1].bottom)-(rc[2].bottom-rc[2].top))*(si.nMax-si.nMin+1-si.nPage);
                if(cur_pos>si.nMax-si.nMin+1-si.nPage) cur_pos=si.nMax-si.nMin+1-si.nPage;
                else if(cur_pos<si.nMin) cur_pos=si.nMin;
                
                char text[256]="";
                sprintf(text,"pre pos:%d, cur pos:%d,pt.y:%d,dragpt.y%d",es->dragpos.y,cur_pos,pt.y,es->dragpt.y);
                ...
                Edit_Scroll(hwnd,(cur_pos-es->dragpos.y),0);
